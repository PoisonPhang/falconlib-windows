...
public class Logging {
    ...
    private FileWriter checkLogCreation() {
        if (Library.connection.checkLogDate()) {
            ...
            try {
                return new FileWriter(String.format(String.format(logSaveDirPath + format.format(Calendar.getInstance().getTime()))));
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            try {
                return new FileWriter((logSaveDirPath + Library.connection.getLastLogDate()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
