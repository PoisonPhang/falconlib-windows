...
public class Logging {
    ...
    public void writeToLog(Enums.LogType logType, String message) {
        if (logType == Enums.LogType.USER_ACTION) {
            writer.println(dateTime.format(Calendar.getInstance().getTime()) + " [ USER EVENT ]: " + message);
            System.out.println(TAG + dateTime.format(Calendar.getInstance().getTime()) + " [ USER EVENT ]: " + message);
        } else if (logType == Enums.LogType.BOOK_ACTION) {
            writer.println(dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK EVENT ]: " + message);
            System.out.println(TAG + dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK EVENT ]: " + message);
        } else if (logType == Enums.LogType.CHECKOUT) {
            writer.println(dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK CHECKED OUT ]: " + message);
            System.out.println(TAG + dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK CHECKED OUT ]: " + message);
        } else if (logType == Enums.LogType.RETURN) {
            writer.println(dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK RETURNED ]: " + message);
            System.out.println(TAG + dateTime.format(Calendar.getInstance().getTime()) + " [ BOOK RETURNED ]: " + message);
        }
    }
    ...
    private FileWriter checkLogCreation() {
        if (Library.connection.checkLogDate()) {
            ...
            try {
                return new FileWriter((( new File(logSaveDirPath ,format.format(Calendar.getInstance().getTime())))), true);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
